...
2. **Мультивкладочный модульный интерфейс**:
   - 🖥️ **Ноды (Nodes)**: Сводка нод, роли (Control Plane / Worker), быстрые действия.
   - 💾 **Хранилище (Storage)**: Физические накопители (`/dev/sda`), размеры, разделы COSI (`/system/state`, `/var`).
   ...
   - ⚡ **Операции (Operations)**: Проверка здоровья кворума etcd, запуск Rolling Reboot, Maintenance Mode.
3. **Живой стриминг логов (WebSocket)**:
   - Стриминг сообщений ядра `dmesg` и логов служб (`kubelet`) с автоскроллом, паузой и выгрузкой в `.log`.
4. **Интеграция с Proxmox VE**:
   - Опрос свободных ресурсов гипервизора Proxmox (CPU, RAM, емкость `local-lvm`).
   - Автоматическое создание ВМ-воркеров (`CreateTalosWorker`) в один клик через REST API.
5. **Резервное копирование и DR (Disaster Recovery)**:
...
6. **Telegram-алертинг**:
   - Фоновый наблюдатель (`Watcher`) с умным дебаунсингом (оповещения только при смене состояний `Ready` ↔ `NotReady` или падении кворума etcd).
7. **Двуязычность и тёмная тема**:
   - Переключатель языков **RU / EN** в шапке с сохранением в `localStorage`.
