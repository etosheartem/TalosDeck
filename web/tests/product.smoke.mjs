...
async function contextFor(role='admin'){
 const context=await browser.newContext({viewport:{width:1440,height:1080}});await context.addInitScript(()=>localStorage.setItem('talosdeck_token','fixture-token'));
 ...
 await context.route('**/api/**',async route=>{
   ...
   else if(path==='/api/backups')result=[backup];
