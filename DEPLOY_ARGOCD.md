...
## Обнаружение нод и язык интерфейса

Панель получает состав кластера из Talos `Members` через адреса подключения в talosconfig. `endpoints` — адреса Talos API, а не полный список машин. Состав обновляется при опросе (кеш 30 секунд); при ошибке discovery сохраняется последний успешный список и явно заданные ноды.

Если discovery отключён или ресурсы `Members` недоступны, укажи все адреса в поле `nodes` выбранного контекста talosconfig и обнови Secret, затем перезапусти Deployment. Альтернатива — `args: ["-nodes=10.0.0.10,10.0.0.11,10.0.0.12"]` у контейнера в Git-манифесте (адреса замени своими). Явно заданные ноды не исчезают из списка при недоступности.

Переключатель **Русский / English** и ссылка на проект GitHub находятся внизу боковой панели. Язык сохраняется в браузере. Содержимое логов, MachineConfig и сообщения внешних API остаются в исходном виде.
